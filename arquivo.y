...
Ast * newtext(int nodetype, char* txt) { /*Função de que cria uma nova variável*/
...
%token <str>VARS
%token <str>TEXTO
%token FIM INICIO IF ELSE WHILE PRINTI PRINTF PRINTS DECL SCANINT SCANFLOAT SCANSTRING SCANS STRING INT FLOAT
%token <fn> CMP
...
| INT VARS { $$ = newvari('V',$2);}
| FLOAT VARS { $$ = newvari('V',$2);}
| STRING VARS { $$ = newvari('V',$2);}
| PRINTF '(' exp ')' { $$ = newast('P',$3,NULL);}
| PRINTS '(' exp1 ')' {$$ = newast('Y',$3,NULL);}
| PRINTI '(' exp ')' {$$ = newast('R',$3,NULL);}
| VARS '=' SCANINT '(' ')' { $$ = newvari('A',$1);}
| VARS '=' SCANFLOAT '(' ')' { $$ = newvari('S',$1);}
| VARS '=' SCANSTRING '(' ')' { $$ = newvari('T',$1);}
